...
### Some background on microbiology

- In cellscript, code is written as DNA, which is a sequence of genes.
...
Source is in cellscript.py (the biological simulation), circt_compile.py (compilation from a circuit) and from_yosys.py (for loading circuit from yosys JSON).

See hello/hello.txt for a DNA sequence for a program that prints hello. It is compiled from hello/hello.sv (SystemVerilog is a hardware definition language, used to
